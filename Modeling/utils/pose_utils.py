...
def gen_clip_seg_data_np(clip_dict, start_ofst=0, seg_stride=4, seg_len=12, scene_id='', clip_id='', ret_keys=False,
    ...
    for idx in sorted(clip_dict.keys(), key=lambda x: int(x)):
        ...
        #key = "{:0>2}_{:0>4}_{:02d}".format(scene_id, clip_id, int(idx))
        ...
        #print(f"gen_clip_segs_data_np\n curr_pose_score_np.shape: {curr_pose_score_np.shape}\n ")
        #print(f"curr_pose_score_np : {curr_pose_score_np}\n ")     
        pose_segs_data.append(curr_pose_segs_np)
        score_segs_data.append(curr_pose_score_np)
        if sing_pose_np.shape[0] >= seg_len: #changed > to >=
    ...
    del pose_segs_data
    #print(f"SHAPE OF score_segs_data_np just before return : {score_segs_data_np.shape}")
...
def single_pose_dict2np(person_dict, idx):
    ...
    single_person_dict_keys = sorted(single_person.keys()) #frame_id ( i added this comment )
    sing_pose_meta = [int(idx), int(single_person_dict_keys[0])]  # Meta is [index, first_frame] (index is the person id) 
